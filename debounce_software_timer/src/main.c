...
void vbuttonTask(void *pvParameter){
    ...
    {  
        if (((GPIOF->DATA  & (1<<4)) == 0x00) && (button1_state == 1))
        {
            ...
            button1_state = 0;
            //set timer ID
            vTimerSetTimerID(xtimer1, (void *)id);
        ...
        if (((GPIOF->DATA & (1<<0)) == 0x00) && (button2_state == 1))
        {
            ...
            button2_state = 0;
            //set timer id
            vTimerSetTimerID(xtimer2, (void *)id);
...
int main(void)
{
    gpio_init();
    //create timer
    xtimer1 = xTimerCreate("button 1", pdMS_TO_TICKS(50), pdFALSE, 0, vtimer_callback);
    xtimer2 = xTimerCreate("button 2", pdMS_TO_TICKS(50), pdFALSE, 0, vtimer_callback);
